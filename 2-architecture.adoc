...
Providers are broadly divided into categories, and in CloudForms 4.5 these are Cloud, Infrastructure, Container, Configuration Management, Automation, Network, Middleware and Storage.
...
CloudForms 4.5 supports Red Hat Satellite as a configuration management provider. The Satellite provider exposes the Foreman host groups to CloudForms, and extends the provisioning capability to include bare-metal (i.e. non-virtual) servers. 

==== Automation Providers

CloudForms 4.5 supports Ansible Tower as an automation provider. The Tower provider allows Ansible jobs to be run on an external Tower server as part of an Automate workflow.
