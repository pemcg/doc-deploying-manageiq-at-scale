...
[NOTE]
====
The Provider Inventory refresh workers roles are described in more detail in <<inventory_refresh>>
====
...
This role enables access to a CFME appliance using the Red Hat CloudForms _Operations_ WebUI console. More than one CFME appliance can have this role in a zone (the default behaviour is to have this role enabled on all appliances). Enabling this server role starts one or more _MiqUiWorker_ workers. 
...
[NOTE]
====
CFME 5.8 has provided a new command that allows the currently running worker processes on the local server _and_ remote servers can be seen, ordered by server and zone:
